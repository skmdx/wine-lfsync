...
static BOOL create_whole_window( struct x11drv_win_data *data, struct x11drv_native_window *native_window )
{
    ...
    sync_window_input_shape( data );
    if (data->client_surface_backing_enabled)
...
static BOOL begin_client_surface_native_barrier( struct x11drv_win_data *data )
{
    BOOL barrier;

    /* Seal the scene before replacing native resources.  The owner compositor
     * drains its own X connection when it destroys the backing. */
    barrier = client_surface_begin_native_barrier( data->hwnd, (UINT_PTR)data );
...
BOOL X11DRV_CreateWindow( HWND hwnd )
{
    if (hwnd == NtUserGetDesktopWindow())
    {
        ...
        data->owns_clip_window = !!data->clip_window;
        XFlush( data->display );
                   ...
                   const RECT *top_rect, DWORD flags, UINT scale_num, UINT scale_den )
{
     ...
     * visible region, without binding an owner drawable in the producer. */
                                  ...
                                  const struct window_rects *new_rects, struct window_surface *surface )
{
    ...
    if (enable_client_surface_backing || disable_client_surface_backing)
    {
        if (enable_client_surface_backing)
        {
            ...
            {
                if (data->client_surface_backing) status = X11DRV_client_surface_backing_ensure( data );
                else status = X11DRV_client_surface_backing_snapshot( data, FALSE );
            }
        }
        else if (!X11DRV_client_surface_backing_retire( data )) destroy_client_surface_backing( data );
    }
