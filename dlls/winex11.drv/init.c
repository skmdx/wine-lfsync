...
static X11DRV_PDEVICE *create_x11_physdev( Drawable drawable )
{
    ...
    physDev->drawable_scale_num = physDev->drawable_scale_den = 1;
        ...
        if (in_data && in_count >= sizeof(enum x11drv_escape_codes))
        {
            ...
            case X11DRV_SET_DRAWABLE:
                if (in_count >= sizeof(struct x11drv_escape_set_drawable))
                {
                    ...
                    physDev->drawable_scale_num = data->drawable_scale_num ? data->drawable_scale_num : 1;
                    physDev->drawable_scale_den = data->drawable_scale_den ? data->drawable_scale_den : 1;
...
static const struct user_driver_funcs x11drv_funcs =
{
    ...
    .pWindowPaint = X11DRV_WindowPaint,
