...
static UINT64 image_memory_available( enum client_surface_memory_class type, BOOL metadata )
{
    ...
    unsigned int purpose = metadata ? CLIENT_SURFACE_MEMORY_CLASS_COUNT : type, i;
    UINT64 committed = 0;

    for (i = 0; i < ARRAY_SIZE(used); ++i)
        committed += max( used[i], image_memory_floor[i] );
    assert( committed <= CLIENT_SURFACE_IMAGE_MEMORY_LIMIT );
    return CLIENT_SURFACE_IMAGE_MEMORY_LIMIT - committed +
           max( used[purpose], image_memory_floor[purpose] ) - used[purpose];
}
...
static BOOL reserve_image_memory( enum client_surface_memory_class type, UINT64 bytes, BOOL metadata )
{
    BOOL ret;
    ...
    trace_client_surface_memory( "image_reserve", type, bytes, ret, metadata );
    pthread_mutex_unlock( &image_memory_lock );
    return ret;
}

static void release_image_memory( enum client_surface_memory_class type, UINT64 bytes, BOOL metadata )
{
    assert( type < CLIENT_SURFACE_MEMORY_CLASS_COUNT );
    pthread_mutex_lock( &image_memory_lock );
