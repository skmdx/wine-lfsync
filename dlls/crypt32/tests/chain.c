...
static void testGetCertChain(void)
{
    ...
    if (chain)
    {
        BOOL full_chain = (chain->TrustStatus.dwErrorStatus &
         ~opensslChainCheck.status.statusToIgnore.dwErrorStatus) ==
...
static void check_ssl_policy(void)
{
     ...
     * with various combinations of matching and non-matching names.  The old
     * GlobalSign root is no longer generally trusted, so ignore unknown CAs.
